...
**影响**：多 session 环境下行为变化。CLI 单 session 不受影响。
...
| ID | 问题 | 修复 | 影响 |
|----|------|------|------|
| CM-001 | MODEL_CONTEXT=180000 超实际 128K | 改为 131072（128K tokens） | 压缩现在会触发 |
...
| PS-006/007 | dominant_emotion 键名不一致 | 统一为 `afraid`（3 文件对齐） | 行为描述不再丢失 |
...
| 风险 | 等级 | 缓解 |
|------|------|------|
| session_id 过滤导致已有数据不可见 | 高 | `WHERE session_id='default'` 保底 |
| CM-001 压缩频率增加性能下降 | 中 | 监控 token 数，必要时调整阈值 |
| 情绪系统修改影响用户体验 | 高 | 充分手动测试，可回滚 |
| WebAgent 共享 Provider 线程安全 | 中 | 加锁 + 测试 |
