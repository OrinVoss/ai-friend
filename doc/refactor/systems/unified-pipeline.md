...
| 期 | 内容 | 依赖 |
|----|------|------|
| **P0 装配统一** | 新建 `SessionFactory`，`main.py` 和 `web/session.py` 都改调它；行为不变，先消灭配置漂移 | 无 |
| **P1 管线统一** | `MessageHandler` 包出 `ConversationEngine` 事件接口；CLI 切换（`cli_shared_pipeline` 开关，灰度后默认开）；删除 CliController 内联 ReAct | P0 |
...
| 已立案问题 | 解决方式 |
|-----------|---------|
| CLI 情绪永不更新（`emotion.md` P0） | 管线只有一条，情绪更新自然覆盖 CLI |
| CLI 无睡眠循环（`cli.md` P0） | RuntimeDriver 共享（P2） |
| `<tool_call>` 原始标记喷给用户（`cli.md` P0） | 引擎只发清理后的事件，前端不再接触原始流 |
| 装配配置漂移（`provider.md`） | SessionFactory 单一接线（P0） |
| Web `session_id` 共享竞态（`web.md` P0） | Factory 为每 session 建独立 Repository（P0） |
...
1. **等价性**：同一输入经引擎产生的事件序列，CLI mock 前端与 Web mock 前端收到的一致
2. SessionFactory：两个 session 的 Repository 互不共享 `session_id`
3. 开关回退：`cli_shared_pipeline=false` 时 CLI 走旧路径行为不变（P1 灰度期间）
