...
> 最后更新：2026-07-12 | 总计 189 issues（85 已完成，104 开放）| v0.1 ✓ | v0.2 ✓ | v0.4 4 issue 收尾 | ✅ 文档同步更新中
...
| v0.2 | 记忆系统升级 | 34 | 9 | 25 | 26% |
| v0.3 | 情感与人格 | 25 | 13 | 12 | 52% |
| v0.4 | Web 工程化 | 16 | 8 | 8 | 50% |
| v0.5 | 前瞻与质量 | 56 | 19 | 37 | 34% |
| v1.0 | 正式版发布 | 9 | 1 | 8 | 11% |
| v2.0 | 远景合并 | 3 | 0 | 3 | 0% |
| 未分类 | — | 16 | 5 | 11 | — |
...
### v0.2 新建（代码审查 #143-#185）

| # | 标题 | 说明 |
|---|------|------|
| #106 | AnySearch API 调用方式不一致 | 参数不统一、安全风险 |
| #109 | WebSocket 分段发送 | 长消息分布不均 |
| #150 | NotifyTool 命令注入 + GrepTool ReDoS | 安全漏洞 |
| #153 | personality.json 写入不原子 | 文件损坏风险 |
| #154 | 数据库架构缺陷 | Schema 缺会话隔离、连接池、边界 |
| #155 | API Key 泄露 + SSRF + Session ID 客户端可控 | 安全漏洞 |
| #156 | web_tools Session 未复用 + 嵌套调用 81 次 API | 性能 |
| #157 | 数据库可靠性 | 连接未关闭、文件损坏、Schema 版本 |
| #158 | WebSocket Origin 验证 + 速率限制 | 安全漏洞 |
| #159 | 情绪事件不持久 + _consecutive_negative 丢失 | 数据丢失 |
| #162 | 异步/同步桥接系统性架构缺陷 | 全文件异步化 |
| #165 | 安全降级模式 + checkpoint/回滚 | 可靠性 |
| #176 | ConversationBuffer 无单条消息大小限制 | OOM 风险 |
...
| #58 | 统一启动入口 | `core/embedding_server.py` + `core/logging_setup.py` 提取 CLI/Web 共享启动代码；入口仍保留 `main.py` / `web_main.py` |
| #24 | Web 安全 | CORS 支持 `config.allowed_origins`；`RateLimitMiddleware` + WebSocket 限流；CSP/X-Frame-Options/X-Content-Type-Options/Referrer-Policy 响应头 |
| #43 | Pydantic 验证 | 新增 `web/schemas.py`：`ChatRequest`/`ChatResponse`/`StatusResponse`/`HistoryResponse`/`HistoryTurn` |
| #45 | Web 封装 | 新增 `WebAgent` 封装 `Agent` 私有接口；`web/server.py` 不再直接访问 `agent._xxx` |
...
| #28 | Prompt 可配置 | ✅ `config.conversation_examples` 对话风格示例可配置化 |
...
| #52 | ARIA/键盘 | 无障碍访问 |
| #53 | CSP 头 | 前端安全头 |
| #54 | CSS 变量 | ✅ 颜色集中管理：`web/static/style.css` 使用 CSS 变量，移除硬编码色值 |
...
现在 ──▶ v0.1 收尾（2 issue：#10 schema + #32 tokenizer）
         ...
         v0.3 情感深化（4 issue）
            ...
            ▼
         v0.4 Web 工程化（8 issue，#58/#24/#43/#45 已关闭）
            │  持久化、统一入口 ✅、安全 ✅、Pydantic ✅、性能、WS ✅
            ▼
         v0.2 记忆升级（6 issue）
...
| #81 | Web 端生产可用 | #58 #57 #44 #24 #43 #46 | 统一入口 ✅、持久化完整、性能、安全 ✅、Pydantic ✅、WebAgent 封装 ✅ |
...
| #84 | 文档完整 | #50 | 全部 11 份文档同步中：API、架构、消息流、技术、配置参考、人格指南、测试指南、工具开发、Prompt 参考、部署、里程碑 |
...
- [~] 所有 v0.1~v0.5 关键 bug 已关闭（v0.1 除 #32 外全部清零；v0.4 #58/#24/#43/#45 已关闭；v0.5 #49 typing_speed 仍未修复）
- [ ] 情感四层架构完整运作
- [~] Web 端可长期运行不泄漏/不崩溃（#57/#44/#46 仍开放）
- [x] 10+ 单元测试通过（171 用例全部通过）
- [~] 文档覆盖所有系统模块（11 份核心文档已更新）
- [~] `python start.py` 统一启动 CLI/Web（#58 已提取共享启动代码，入口仍分离）
