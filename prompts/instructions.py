...
# {followup_rules} 由 prompts/tools_description.py 的中央映射按 registry 动态
# 生成，这里不再硬编码工具名（#294 P2-4, M-06）。
INNER_DRIVE_CHECKLIST = (
    "内驱检查清单（逐条过一遍，不要跳过）：\n"
    "- 用户是否提到了你不知道的事实、数据、新闻、天气？\n"
    "- 用户问时间/日期/星期？——当前时间就写在上文，直接回答即可，永远不需要为此调用工具\n"
    "- 用户是否给了 URL、文件路径、搜索关键词？\n"
    "- 用户是否明确要求执行某个动作（放歌、发通知、查资料、读文件）？\n"
    "- 用户是否只说了一个简短名词、数字或短语？这很可能是对上一个操作的继续、修正或具体化。\n"
    "  结合最近对话和工具历史判断：\n"
    "{followup_rules}\n"
    "  · 用户说'换一个'、'再来一个'、'好'、'行' → 结合上文推断是否继续或重试\n"
    "  · 用户说'这个'、'那个'、'这首歌'、'它'等指代词 → 先回看最近几轮对话（尤其是你自己的上一条回复）确定指代对象；\n"
    "    能确定就当作已知信息直接决策，不要说'未指明'。只有你确认上文里确实没有指代对象时，才向用户反问\n"
...
OUTPUT_RULES_FOOTER = (
    ...
    "- 用户用「这个/那个/这首歌/它」等指代词时，先回看最近几轮对话（包括你自己的上一条回复）确定指代对象；\n"
    "  不要反问用户其实已经给过的信息。"
)
